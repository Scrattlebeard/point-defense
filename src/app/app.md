...
| File | Purpose |
|------|---------|
| `main.js` | Entry point: boot, mode state machine (`menu / play / levelup / pause / over`), rAF loop with clamped dt, visibility auto-pause. **Canvas sizing invariant:** game coordinates are CSS pixels (`G.W/G.H = innerWidth/innerHeight`); the backing store is `×dpr` (capped at 2) with a matching `ctx.setTransform`; and the canvas's *displayed* size must be pinned to the viewport in CSS (`#field { width/height: 100% }`) — a canvas is a replaced element, so `inset: 0` alone does NOT stretch it; `width: auto` resolves to the intrinsic (backing-store) size, which on dpr>1 screens showed only the upper-left `1/dpr²` of the field (2026-07-23, first phone run off the artifact wrapper). **Phone zoom:** when `min(innerWidth, innerHeight) < 600`, the world renders at `PHONE_ZOOM = 1.3×` — game coords become `innerWidth/zoom × innerHeight/zoom`, the ctx transform carries `dpr·zoom`, and input maps through automatically (it already scales by `G.W/rect.width`). Entities, HUD-on-canvas and gesture feel all grow together; DOM overlays are untouched. *Recorded assumption:* this shrinks the logical field ~23% on phones vs. what the 2026-07-23 playtest balance was tuned on (shorter travel distances, combat-radius covers more of the field) — accepted as the point of the change (thumb-sized targets beat exact field parity); revisit via the balance-round-2 pin if deep waves feel cramped. The breakpoint keys on min-dimension so rotating a phone never re-zooms mid-run. Re-syncs on `resize` (covers orientation change and mobile URL-bar show/hide) |
