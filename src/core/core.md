...
| File | Purpose |
|------|---------|
| `geom.js` | Vector helpers: `dist`, `distToSegment`, `clamp`, `lerp`, angle utils, `bladeSpine` (the orbit blade's swept centreline — **shared by the hit test and the renderer**, so the drawn curve is the bitten curve) |
...
| id | max | behavior |
|----|-----|----------|
| orbit | 5 | **radial blades sweeping around the Point** — **contact damage**: a blade grinds any shape it touches (they never shoot; the card text must say so — 2026-07-23 second playtester read "blade orbits" and waited for it to fire). A blade is a **spoke**: rooted at `inner` **22** — **on the Point's hull** (TOWER_R 24), fixed at every level, reaching out to `outer` **136/150/163/175/186** — the tip extending is what levelling buys — `reach` 9 across, and **swept back 0.32 rad** — the tip lags the root, so the blade curves against its own rotation instead of reading as a rod (Daniel, 2026-07-26: *"feels a bit like whacking the enemies with a rod"*). The sweep is back-loaded (`t^1.35`): the root leaves the hub straight and the curve is felt at the business end. Blade widths are asymmetric — a thin honed edge leading, the mass of the blade behind it, both tapering to nothing at the tip; **constant width down a straight spoke was the rod**. The silhouette is a **thin shaft at the hub, a belly about a third of the way out, a point at the tip**: mounting the blades on the hull made a max-at-root taper untenable, because six wide roots at r=22 fuse into a disc and swallow the tower.

**Mounting them cost almost nothing, and why is worth keeping.** Extending the root from 96 to 22 raised orbit's damage in a full maxed build by ~1% (3.70M → 3.75M over 4×15min). The added length sweeps the region the death-radius distribution says is empty — *nothing dies inside 120px*. The same change reads as an **18× buff at L1** in the fixed-build harness, because that harness gives the tower 1e9 HP and shapes pile onto the hull, which is the one situation where the inner blade is loaded. **The two readings are both correct and they answer different questions:** what orbit contributes in a healthy run, and what it does when shapes have already reached you. It is now a very strong last-ditch defence — see the Law·Delegation pin. Count **2/3/4/5/6**; dmg **10+4L**; per-shape bite cooldown **0.80/0.66/0.52/0.40/0.31s**; angular speed 2.3+0.18L; outward shove 30. *(ADR-0022, 2026-07-26, superseding the tangential arc-blade of ADR-0020/0021 the same day. Four rules hold and are tested: the blade ROOT never moves, every level grants a blade or reach, the root stays inside frost's weakest aura, and the blades clear the hub while staying inside a 430px screen's half-width. Three constraints found by measurement and worth not re-learning: blade **count** multiplies how fast a shape is re-cut while **reach** multiplies how long it stays in contact; count **saturates against the bite cooldown**, so the two curves are coupled and re-tuning either demands re-measuring the other — a cooldown that binds at every level silently kills the rungs that grant blades; and a radial blade holds a shape for its whole approach (~5 bites) where the old tangential one caught it crossing a thin band (~1.4), which is why per-bite damage is smaller here and per-crossing damage is nearly double.)* |
