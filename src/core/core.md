...
| id | max | behavior |
|----|-----|----------|
| orbit | 5 | **blades orbiting the Point on a fixed ring** — **contact damage**: a blade grinds any shape it touches (they never shoot; the card text must say so — 2026-07-23 second playtester read "blade orbits" and waited for it to fire). A blade is an **arc of the ring**, not a point on it: `len` long along the circumference, `reach` 13 thick, **raked** so its leading end rides further out than its trailing heel and honed along that leading edge (ADR-0021 — a symmetric blade travelling along its own axis reads as a spike being carried, not a knife cutting; the rake is drawing only, the hit test is still the straight chord). Ring radius **138 at every level**; count **2/3/4/5/6**; length **8/14/22/30/38**; dmg **17+9L**; per-shape bite cooldown **0.50/0.46/0.42/0.36/0.30s**; angular speed 2.3+0.18L; outward shove **30** *(was 45)*. *(All of this re-shaped 2026-07-26 — ADR-0020, which supersedes the 2026-07-24 and 2026-07-25 radius work. Three rules now hold and are tested: the ring never moves, arc coverage never falls as the weapon levels, and the blades' inner edge stays inside frost's weakest aura at every level of both weapons. The old ladder broke all three — coverage peaked at L2 and declined for the rest of the ladder, because a fixed 13px blade sat beside a growing radius, and the ring walked out of the aura at L4. Measured consequence: the L3 rung stepped +41% where its neighbours stepped +130%. Two constraints found by measurement and worth not re-learning: blade **count** multiplies how fast a shape is re-cut while blade **length** only multiplies whether it is cut at all — length cannot carry a level — and count saturates against the bite cooldown, which is why that cooldown is a ladder and not a constant.)* |
